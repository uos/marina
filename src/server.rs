...
use mt_dataset::registry::driver::RegistryDriver;
use mt_dataset::registry::minot::protocol::{
...
pub async fn serve(driver: Arc<dyn RegistryDriver>, options: ServeOptions) -> Result<()> {
    ...
    let materialize_root = match options.materialize_root {
        Some(root) => root,
        None => mt_dataset::storage::config::cache_dir()
...
impl RequestHandler {
    async fn record_access(&self, bag: &mt_dataset::model::bag_ref::BagRef) {
    ...
    fn dataset_dir(&self, bag: &mt_dataset::model::bag_ref::BagRef) -> std::path::PathBuf {
    ...
    async fn materialize(
        ...
        tokio::task::spawn_blocking(move || {
            mt_dataset::io::pack::unpack_bag(&staging_path, &incoming_for_task)
        })
        .await
    ...
    async fn remove(&self, bag: mt_dataset::model::bag_ref::BagRef) -> Result<Response, String> {
        self.ensure_writes_enabled()?;
    ...
    fn packed_push_path(&self, bag: &mt_dataset::model::bag_ref::BagRef) -> std::path::PathBuf {
    ...
    async fn commit_push(
        ...
        let _guard = self.write_lock.lock().await;
        let path = self.packed_push_path(&bag);
        let meta: mt_dataset::registry::driver::PushMeta = wire_meta.into();
    ...
    fn write_staging_dir(&self, bag: &mt_dataset::model::bag_ref::BagRef) -> std::path::PathBuf {
    ...
    async fn commit_write(
        ...
        use mt_dataset::io::pack::{ArchiveCompression, PackOptions};
        use mt_dataset::registry::driver::PushMeta;
        use mt_dataset::storage::cache::MirrorFile;
        use sha2::{Digest, Sha256};
        ...
        let actual = mt_dataset::storage::cache::mirror_manifest(&source_dir)
        ...
        mt_dataset::io::bag::discover_bag(&source_dir)
        ...
        let packed_meta = tokio::task::spawn_blocking(move || {
            let source = mt_dataset::io::bag::discover_bag(&ready_for_pack)?;
            let mut progress = mt_dataset::ProgressReporter::silent();
            mt_dataset::io::pack::pack_bag_with_progress_and_options(
                &source,
                &packed_for_task,
                PackOptions {
                    transform: mt_dataset::io::mcap_transform::PushTransformOptions {
...
pub fn clean_streaming_cache(max_age: Duration) -> Result<CacheCleanSummary> {
    let materialize_root = mt_dataset::storage::config::cache_dir()
        .context("could not locate the cache directory")?
        .join("serve");
