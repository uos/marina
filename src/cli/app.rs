...
use mt_dataset::progress::{ProgressReporter, WriterProgress};
...
fn complete_datasets(current: &std::ffi::OsStr) -> Vec<CompletionCandidate> {
    ...
    let Ok(marina) = mt_dataset::core::Marina::load() else {
        return vec![];
    };
...
async fn run_parsed(cli: Cli, raw_yes: bool) -> Result<()> {
    ...
    match cli.cmd {
        ...
        Commands::Finalize(args) => {
            ...
            mt_dataset::io::bag::discover_bag(&ready).with_context(|| {
                format!(
        ...
        Commands::CacheReceive(cmd) => match cmd.cmd {
            CacheReceiveSub::Prepare { bag } => {
                let plan = mt_dataset::storage::cache::prepare_mirror_receive(&bag)?;
                println!("{}", serde_json::to_string(&plan)?);
            }
            CacheReceiveSub::Commit { bag } => {
                let path = mt_dataset::storage::cache::commit_mirror_receive(&bag)?;
                println!("{}", path.display());
            }
        },
        ...
        Commands::Serve(args) => {
            ...
            let config = marina
                ...
                .clone();
            let driver = mt_dataset::core::marina::make_registry_driver(&config)?;
            let exposed_as = args.r#as.clone().unwrap_or_else(|| name.clone());

            let mut options = crate::server::ServeOptions::new(exposed_as.clone());
            ...
            crate::server::serve(driver, options).await?;
        }
        Commands::Version => {
...
fn print_inspect_result(result: &mt_dataset::core::InspectResult) {
    let is_tty = std::io::stdout().is_terminal();
    let time_display = config::load_registries()
        .map(|f| f.settings.time_display)
        .unwrap_or_default();
